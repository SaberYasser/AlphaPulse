...
class SymbolEngine:
    ...
    def _evaluate_trend(self, now_ts: float, price: float) -> None:
        ...
        Evaluated at 1 Hz from rolling aggregates (O(1)): a meaningful net
        move over ~trend_window_s, carried by dominant one-sided volume above
        the minute-of-day baseline, while price makes a new N-minute extreme.
        Fires through StateMachine.consider_trend (shared caps/cooldowns).
        """
        cfg = self.cfg.engine
        if not cfg.trend_detector or self.catchup or not self.alerts_enabled():
            return
        if self.session_open_ts <= 0.0 or now_ts < self.session_open_ts + cfg.trend_window_s:
            return
        ago = self.agg.price_ago(cfg.trend_window_s)
        if ago is None or ago <= 0.0:
            return
        move_bps = (price - ago) / ago * 1e4
        if abs(move_bps) < cfg.trend_min_bps:
            return
        direction = 1 if move_bps > 0 else -1
        v60, b60, s60, _n60, _d60 = self.agg.window(60)
        classified = b60 + s60
        if classified <= 0:
            return
        share_dir = (b60 / classified) if direction > 0 else (s60 / classified)
        if share_dir < cfg.trend_dir_share_min:
            return
        minute_idx = (
            minute_of_session(now_ts, self.session_open_ts) if self.session_open_ts > 0 else 0
        )
        base_min = self.baseline.minute_volume(self.symbol, minute_idx)
        ...
        pseudo = Trade(symbol=self.symbol, ts=now_ts, price=price, size=0, conditions=("@",))
        snap = self._snapshot(pseudo, Level(price=ref, kind=kind), direction, is_reclaim=False)
        snap.trigger_verb = "trend"
        snap.is_trend = 1.0
        if base_min > 0:
            # For an escalator the meaningful volume figure is the sustained
            # 60s participation vs baseline, not the 5s burst ratio (~1x by
            # nature of the class). Recorded on the signal and shown in the
            # message; the model separates contexts via the "trend" flag.
            snap.vol_ratio_prev = v60 / base_min
        self.machine.consider_trend(snap)
